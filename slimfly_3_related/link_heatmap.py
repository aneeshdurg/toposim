        ...
        for pkt in packets:
            if pkt.haslayer("Ether"):
                ...
                if src_mac in mac_to_names.keys() and dst_mac in mac_to_names.keys():
                    traffic_matrix[mac_to_names[src_mac]][mac_to_names[dst_mac]] += len(
                        pkt
                    )
...
def create_heatmap(traffic_matrix):
    ...
    plt.colorbar(label="Traffic Size (bytes)").set_label(
        "Traffic Size (bytes)", rotation=270, labelpad=20
    )
    ...
    if re.match("slimfly_3_r", src):
        for dst, iface in dst_ifaces.items():
            if re.match("slimfly_3_r", dst):
                pcap_files.append(
                    f"/mnt/dhwang/tcpdump_outputs/dumpfile_{src}_{iface}.pcap"
                )
